...
// ⚠️ DirectByteBuffer 流接口（android-p7zip 修复，2026-08-17 移植）：
// 用 ByteBuffer 代替 byte[] 传递流数据，Native 层 NewDirectByteBuffer 直接包装
// native 内存 → 零数组创建、零复制、零 JNI 局部引用泄漏（原版 read(byte[]) 在
// 压缩密集读取时打爆 JNI local reference table → SEGV_ACCERR）。
